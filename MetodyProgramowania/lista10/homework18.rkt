...
(struct flip-expr  (p x y)          #:transparent)
(struct uniform-expr (xs)           #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(app-expr ef ea)    (and (expr? ef)
                              (expr? ea))]
    [(flip-expr p x y)        (and (expr? p) (expr? x) (expr? y))]
    [(uniform-expr xs)        (expr? xs)]
    [_                   false]))
...
(define (parse q)
  (cond
   ...
   [(and (symbol? q)
         (not (keyword q)))
    (var-expr q)]
   [(and (list? q) (eq? (length q) 4) (eq? (first q) 'flip))
        (flip-expr (parse (second q)) (parse (third q)) (parse (fourth q)))]
   [(and (list? q) (eq? (length q) 2) (eq? (first q) 'uniform))
        (uniform-expr (parse (second q)))]
   [(and (list? q)
         (= (length q) 3)
         (eq? (first q) 'let)
         (list? (second q))
...
(define (builtin/1 p)
  (builtin (lambda (x) (return (cons (car x) (p (cdr x))))) null 1))
(define (builtin/2 p)
  (builtin (lambda (x y) (return (cons (* (car x) (car y))(p (cdr x) (cdr y))))) null 2))
...
(define (eval-env e env)
  (match e
    ...
    [(var-expr x)
     (return (cons 1 (env-lookup x env)))]
    [(flip-expr p x y)
        (set-mcar! (eval-env x env) (mcdr (eval-env p env)))]
    [(let-expr x e1 e2)
    ...
    [(app-expr ef ea)
     ...
     (bind (cdr (eval-env ea env)) (lambda (va)
       (match vf
         [(clo x e env)
...
(eval (parse '(+ 2 1)))
;(parse '(uniform (cons 1 (cons 2 (cons 3 (cons 4 (cons 5 (cons 6 null))))))))