...
(struct fail-expr (smth)            #:transparent)
  ...
  (cond
   ...
   [(and (list? q) (eq? (first q) 'list))
        (define (pars-iter x)
            (if (null? (cdr x))
                (parse (car x))
                (list (parse (car x)) (pars-iter (cdr x)))))
        (pars-iter (cdr q))]
   [(and (list? q) (eq? (first q) 'fail))
        (fail-expr (parse (second q)))]
   [(and (symbol? q)
         (not (keyword q)))
    (var-expr q)]
   [(and (list? q)
...
(define effect-env
  (foldl (lambda (p env) (env-add (first p) (second p) env))
         start-env
         `((choose ,(effect-builtin/1 (lambda (x) (list x))))
           )))
...
;;;;;;44444 ~ dodajemy fail ~ i dziala XD
(eval (parse '(+ 12 2)))

(eval (parse '(if (> (+ 21 3123) (- 432577 890)) 997 (fail 0))))


;;;;;;555555
effect-env
(eval (parse '(choose (list 2 3))))