...
(define (l-helper q)
    (cond 
        [(null? q) (null-expr )]
        [else (cons-expr (parse (car q)) (l-helper (cdr q)))]))
...
(define (eval-env e env)
  (match e
    ...
    [(null-expr) null]
    [(apply f li)
     (let ([ev (lambda (x) (eval-env x env))]) (ev (app-help f (reverse (ev li)))))]
...
(define (app-help f l)
  (if (null? (cdr l))
    (app f (const (car l)))
    (app (app-help f (cdr l)) (const (car l)))))
...
(define sample '(apply (lambda (x y) (- x y))(list 1 2)))
(define sample2 '(apply (lambda (x y z) (+ x (+ y z)))(list 1 2)))
(define sample3 '(apply (lambda (x y) (lambda (z) (+ x (+ y z))))(list 1 2 3)))
(define sample4 '(apply (lambda (x y) (+ x y))(list 1 2 3)))
(define sample5 '(apply (lambda (x y z)(+ x y)) (if true (list 1 2 3) (lista 4 5 6))))
(ep sample)
(ep sample2)
(ep sample3)
(ep sample5)
