...
int ext2_block_used(uint32_t blkaddr) {
  if (blkaddr >= block_count)
    return EINVAL;
  /* DONE */
  long bg_id = (blkaddr - first_data_block) / blocks_per_group;
  long blkidx = (blkaddr - first_data_block) % blocks_per_group;
  int bitmapblk = group_desc[bg_id].gd_b_bitmap;
  blk_t *blk = blk_get(0, bitmapblk);
  char *bitmap = blk->b_data + blkidx / 8;
  int used = (*bitmap) >> (blkidx % 8) & 1;
  blk_put(blk);
  return used;
}
...
int ext2_inode_used(uint32_t ino) {
  if (!ino || ino >= inode_count)
    return EINVAL;
  /* DONE */
  long bg_id = (ino - 1) / inodes_per_group;
  long ind_idx = (ino - 1) % inodes_per_group;
  int inomapblk = group_desc[bg_id].gd_i_bitmap;
  blk_t *blk = blk_get(0, inomapblk);
  char *bitmap = blk->b_data + ind_idx / 8;
  int used = bitmap[0] >> (ind_idx % 8) & 1;
  blk_put(blk);
  return used;
}
...
static int ext2_inode_read(off_t ino, ext2_inode_t *inode) {
  /* DONE */
  int err = ext2_inode_used(ino);
  if (!err || err == EINVAL)
    return ENOENT;
  long ino_idx = (ino - 1) % inodes_per_group;
  long bg_id = (ino - 1) / inodes_per_group;
  int blkaddr = group_desc[bg_id].gd_i_tables;
  blk_t *blk = blk_get(0, blkaddr + ino_idx / 8);
  memcpy(inode, blk->b_data + ino_idx % 8 * sizeof(ext2_inode_t),
         sizeof(ext2_inode_t));
  blk_put(blk);
  return 0;
}
...
static uint32_t ext2_blkptr_read(uint32_t blkaddr, uint32_t blkidx) {
  ...
  if (blkidx < 268) {
    ...
    i = blk->b_data;
    blk_put(blk);
    blk = blk_get(0, i[(blkidx - 268) / 256]);
    i = blk->b_data;
    res = i[(blkidx - 268) % 256];
  } else if (blkidx < 16843020) {
    i = blk->b_data + (blkidx - 65804) / 65536;
    blk_put(blk);
    blk = blk_get(0, (*i));
    i = blk->b_data;
    blk_put(blk);
    blk = blk_get(0, i[((blkidx - 65804) % 65536) / 256]);
    i = blk->b_data;
    res = i[((blkidx - 65804) % 65536) % 256];
  }
  blk_put(blk);
  return res;
}
...
int ext2_read(uint32_t ino, void *data, size_t pos, size_t len) {
  /* DONE */
  size_t idx = pos / BLKSIZE;
  pos %= BLKSIZE;
  if (ino && idx + len / BLKSIZE > block_count) {
    return EINVAL;
  }
  ...
  if (blk == BLK_ZERO)
    ...
    memcpy(data, blk->b_data + pos, len);
    blk_put(blk);
  }
  return 0;
}
...
int ext2_readdir(uint32_t ino, uint32_t *off_p, ext2_dirent_t *de) {
  /* DONE */
  ext2_inode_t inod;
  ext2_inode_read(ino, &inod);
  if ((*off_p) >= inod.i_size)
    return 0;
  uint32_t ind = 0;
  uint32_t offc;
  while (!ind) {
    ...
    offc = *off_p;
    (*off_p) += de->de_reclen;
    if ((*off_p) >= inod.i_size && ind == 0)
      return 0;
  }
  ext2_read(ino, de->de_name, offc + de_name_offset, de->de_namelen);
...
int ext2_lookup(uint32_t ino, const char *name, uint32_t *ino_p,
  ...
  int out = 0;
  while (1) {
    out = ext2_readdir(ino, &off, &drnt);
    if (strncmp(drnt.de_name, name, drnt.de_namelen + 1) == 0) {
      *ino_p = drnt.de_ino;
      if (type_p)
        *type_p = drnt.de_type;
      return 0;
    } else
      memset(drnt.de_name, 0, 256);
    if (!out)
      break;
  }
  return ENOENT;
...
int ext2_mount(const char *fspath) {
  ...
  group_desc_count = 1 + sb.sb_bcount / sb.sb_bpg;
  ...
  ext2_read(0, group_desc, EXT2_GDOFF,
            sizeof(ext2_groupdesc_t) * group_desc_count / 2);
  ext2_read(0, group_desc + 32, EXT2_GDOFF + EXT2_SBOFF,
            sizeof(ext2_groupdesc_t) * group_desc_count / 2);
  return 0;
}
