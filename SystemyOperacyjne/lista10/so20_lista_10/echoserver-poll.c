...
int main(int argc, char **argv) {
  ...
  while (!quit) {
    ...
    if(fds[0].revents&POLLIN)
    {
      ...
      connfd = Accept(fds[0].fd, (SA *)&clientaddr, &clientlen);
      Getnameinfo((SA*)&clientaddr,clientlen,hostbuf,MAXLINE,servbuf,MAXLINE,flags);
      addclient(connfd,hostbuf,servbuf);
      nready--;
    }
