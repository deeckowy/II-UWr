...
static void db_open(db_t *db, const char *name, size_t size) {
  ...
  int fd = Open(name, O_RDWR | O_CREAT | /*(size ? O_EXCL : 0)*/ 0, DB_MODE);
  ...
  Ftruncate(fd,size);
  db->name=malloc(sizeof *name);
  db->entry=Mmap(NULL,size,PROT_READ|PROT_WRITE,MAP_PRIVATE,fd,0);
  db->size=size;
  Madvise(db->entry,size,MADV_RANDOM);
  Close(fd);
}
...
static bool db_rehash(db_t *db, size_t new_size) {
  ...
  Madvise(db->entry,db->size,MADV_SEQUENTIAL);
...
static void doit(const char *path, op_t mode) {
  ...
  if (isatty(STDIN_FILENO)) {
    ...
    struct stat sb;
    fstat(0,&sb);
    char *inpt=Mmap(NULL,sb.st_size,PROT_READ,MAP_PRIVATE,STDIN_FILENO,0);
    char buf[ENT_LENGTH+1];
    long pos=0;
