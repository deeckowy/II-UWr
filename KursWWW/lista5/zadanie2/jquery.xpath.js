...
(function() {
    ...
    cQuantifiedExpr.prototype.evaluate = function(oContext) {
        ...
        (function(oSelf, nBinding) {
            var oBinding = oSelf.bindings[nBinding++],
                oSequence1 = oBinding.inExpr.evaluate(oContext),
                sUri = (oBinding.namespaceURI ? '{' + oBinding.namespaceURI + '}' : '') + oBinding.localName;
            for (var nIndex = 0, nLength = oSequence1.length;
                (nIndex < nLength) && (bEvery ? bResult : !bResult); nIndex++) {
                oContext.pushVariable(sUri, oSequence1[nIndex]);
                if (nBinding < oSelf.bindings.length)
                    arguments.callee(oSelf, nBinding);
                else
    ...
    function fComparisonExpr_GeneralComp(oExpr, oContext) {
        ...
        for (var nLeftIndex = 0, nLeftLength = oLeft.length, bLeft, vLeft;
            ...
            for (var nRightIndex = 0, nRightLength = oRight.length, bRight, vRight;
                (nRightIndex < nRightLength) && !bResult; nRightIndex++) {
    ...
    cOrExpr.prototype.evaluate = function(oContext) {
        ...
        for (var nIndex = 0, nLength = this.items.length;
            (nIndex < nLength) && !bValue; nIndex++)
    ...
    function fKindTest_parse(oLexer, oStaticContext) {
        ...
        if (oLexer.peek(1) == '(') {
            ...
            if (oLexer.peek() != ')') {
                if (sName == "document-node") {} else
                if (sName == "element") {} else
                if (sName == "attribute") {} else
                if (sName == "processing-instruction") {} else
                if (sName == "schema-attribute") {} else
    ...
    hStaticContext_operators["intersect"] = function(oSequence1, oSequence2) {
        ...
        for (var nIndex = 0, nLength = oSequence1.length, oItem, bFound; nIndex < nLength; nIndex++) {
            ...
            bFound = false;
            for (var nRightIndex = 0, nRightLength = oSequence2.length;
                (nRightIndex < nRightLength) && !bFound; nRightIndex++) {
    ...
    fStaticContext_defineSystemFunction("distinct-values", [
        ...
        for (var nIndex = 0, nLength = oSequence1.length, vLeft; nIndex < nLength; nIndex++) {
            ...
            if (vLeft instanceof cXSUntypedAtomic)
                vLeft = cXSString.cast(vLeft);
            for (var nRightIndex = 0, nRightLength = oSequence.length, vRight, bFound = false;
                (nRightIndex < nRightLength) && !bFound; nRightIndex++) {
